...
const sheetData = require("./data/sheet.js");
...
const GoogleSheet = function(sheetReference, sheetName) {
  ...
  self.build = function() {
    ...
    function createBlips(__, tabletop) {
      try {
        ...
        console.log(JSON.stringify(blips, null, 3));
        plotRadar(
          tabletop.googleSheetName + " - " + sheetName,
          blips,
          sheetName,
          tabletop.foundSheetNames
        );
      } catch (exception) {
  ...
  function createBlipsForProtectedSheet() {
    const sheetNames = ["View all data"];
    const sheetName = sheetNames[0];

    const all = sheetData.values;
    const header = all.shift();
    var blips = _.map(all, blip =>
      new InputSanitizer().sanitizeForProtectedSheet(blip, header)
    );
    plotRadar("TechRadar", blips, sheetName, sheetNames);
  }
...
function setDocumentTitle() {
  document.title = "Intracto TechRadar";
}
...
function plotLoading(content) {
  ...
  plotInitialLoading(content);
}

function plotInitialLoading(content) {
  content
    .append("div")
